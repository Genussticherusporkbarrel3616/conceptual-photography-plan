...
它优先寻找像“被发现”而不是“被设计”的关系：**熟悉物件 × 原生动作或材料行为 × 精确位置 × 极少改造**。所有成功画面都保留同一链条：**原物身份 → 真实连接或动作 → 可见功能结果**。
...
3. **扩展并筛选：**生成候选，通过六项硬门槛，再按趣味性 70%、情绪或含义 20%、视觉冲击力 10% 选优。
4. **完成视觉工程：**明确连接点、承重、材料行为、动作路径、光线及“之前—接触—之后”的单帧证据。
5. **生成或降级：**调用当前平台实际可用的图片工具，分别生成 3:4 全画幅成图；没有图片工具时，分别交付可直接使用的生成提示词，并说明未生成图片。
6. **检查并交付：**对实际成图做盲读，并审计皮肤、材质、非对称、光源、接触阴影、镜头与调色中的 AI 痕迹；只允许一次针对缺失证据的精准修正。
...
```text
熟悉物件 × 原生动作或材料行为 × 精确位置 × 极少改造
```

例如，割草机与眉毛共享“修剪”，雨刷与眼泪共享“擦除”，融化冰块既形成戒指角色，也用原生材料变化提供含义。只靠通用颜色或轮廓相似的组合会被优先淘汰。

### 2. 先否决，再评分

候选必须先通过物件辨识、动作或角色完整、天然联系、物理或角色闭环、摄影美感和原创距离六项硬门槛；任何关键结构不成立，都不能靠偏好分补救。
...
一眼可读性、原创距离、物理或角色逻辑、摄影美感与可拍摄性仍是权重之外的硬门槛。
